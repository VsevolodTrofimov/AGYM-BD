...
var PageTransitions = (function() {
	...
	function init() {
		...
		if(slides[0].beforeSwitch){
			slides[0].beforeSwitch();
		}
	...
	slideAction = function() {
		if(slides[current]) {
			slides[current].actionsCount++;
			if(slides[current].actions) {
				if(slides[current].actions["a"+slides[current].actionsCount]) {
					slides[current].actions["a"+slides[current].actionsCount]();
				}
			}
		}
	}
    ...
    $pages.eq( current ).css('opacity', 1);
    $pages.eq( current+1 ).css('opacity', 1);
		...
		console.log(current, slides[current], $currPage);
		if(slides[current]) {
			slides[current].actionsCount = 0;

			if(slides[current].beforeSwitch) {
				slides[current].beforeSwitch();
			}
		}
		if(slides[(current-1)]&&slides[(current-1)].transition) {
				animation = slides[(current-1)].transition;
		}else {
			animation = "slideLeft";
		}
    ...
    $pages.eq( current-1 ).css('opacity', 1);
		...
		if(slides[current]&&slides[current].transition) {
				animation = slides[current].transition;
		}else {
			animation = "slideLeft";
		}
